...
* main之后:
	...
	3. 执行rootController的相关生命周期方法：
		* init: 在设置rootController时就已创建执行，如果是storyboard加载则会执行initWithNibName->initWithCoder
		* loadView: 加载view,先加载nib文件(nib文件会执行awakeFromNib方法)如果没有则创建空白的view对象作为控制器的主view
