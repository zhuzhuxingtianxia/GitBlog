...
答案：**D**
分析：
block1: block捕获外部变量，引用+1,block->FuncPtr(block)此时block作为型参传递变量引用再次+1;
__weak block2: block捕获外部变量，引用+1,block->FuncPtr(block)此时block未若引用，引用不增加;
block3: copy的block2,只在捕获的引用+1，作为型参传递不变；
block4: __block修改变量，变量转换成了结构体，并持有原变量对象，此时引用+1，block捕获的则是结构体地址指针，FuncPtr传参时结构体作为型参传递,对结构体地址指针做object_assign处理，而原变量引用不变
